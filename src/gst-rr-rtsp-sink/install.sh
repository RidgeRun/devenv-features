...
function check_option() {
    ...
    if [ -z "$var" ]; then
        echo "Please pass the \"${opt_name}\" option to the \"gst-rr-rtsp-sink\" feature" >&2
        exit 1
    fi
}
...
SOURCE_SCRIPT="$SCRIPT_DIR/build-gst-rr-rtsp-sink.sh"
TARGET_SCRIPT="/usr/local/bin/build-gst-rr-rtsp-sink"
...
tmp_script="$(mktemp /tmp/build-gst-rr-rtsp-sink.XXXXXX)"
