...
function check_option() {
    ...
    if [ -z "$var" ]; then
        echo "Please pass the \"${opt_name}\" option to the \"gst-rr-rtsp-sink\" build script" >&2
        exit 1
    fi
}
...
while [ "$#" -gt 0 ]; do
    case "$1" in
        ...
        --help)
            cat <<USAGE
Usage: build-gst-rr-rtsp-sink [--repo-url URL] [--version REF] [--build-type TYPE] [--extra-args ARGS]
USAGE
            exit 0
            ;;
        *)
...
workdir="$(mktemp -d /tmp/gst-rr-rtsp-sink.XXXXXX)"
